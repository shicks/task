...
sub ::PARALLEL :ATTR(CODE) {
  ...
  *{$globref} = sub {
    return _run_parallel($original, $procs, @_);
  };
}
  ...
  while ($parallel->{'submitted'} < $numjobs or %procs) {
    ...
    while ($parallel->{'submitted'} < $numjobs and keys %procs < $numprocs) {
      ...
      if ($pid) {
        ...
        close $reader;
        $parallel->{'done'} = -1;
        # TODO(sdh): try/recover and exit nonzero?
        my $result = $original->($parallel);
    ...
    my $reader = $pipes{$pid};
    local($/) = undef;
    my $result = decode_json(<$reader>)->[0];
    close $reader;
    delete $procs{$pid};
    delete $pipes{$pid};
...
sub index {
  my $self = shift;
  return $self->is_done ? $self->{'done'} : $self->{'submitted'};
}
